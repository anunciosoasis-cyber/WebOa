...
const CountdownLiveModal = ({ onClose }) => {
    ...
    const handleStart = async () => {
        ...
        if (total <= 0) return;
        const targetTime = Date.now() + total * 1000;
        const config = { minutes, seconds, message, subMessage, selectedPreset, customBg };
        const countdownPayload = {
            mode: 'countdown',
            title: message,
            content: subMessage,
            template: 'glass_center',
            targetTime,
            bg_color: currentPreset.bg,
            text_color: '#ffffff',
            accent_color: currentPreset.accent,
            selectedPreset,
            customBg
        };

        // Guardar config en localStorage ANTES de abrir ventanas
        try {
            localStorage.setItem('oasis_countdown_config', JSON.stringify(config));
        } catch {
            // Si la imagen es muy grande, guardar sin ella
            localStorage.setItem('oasis_countdown_config', JSON.stringify({ ...config, customBg: null }));
        }
        localStorage.setItem('obs_overlay_data', JSON.stringify(countdownPayload));

        // Abrir ventanas PRIMERO (leen localStorage inmediatamente)
        // /countdown-live → CountdownPage que renderiza la personalización completa (fondo, mensajes, etc.)
        window.open('/countdown-live', 'COUNTDOWN_PROYECTOR_WINDOW', 'width=1920,height=1080');
        // /transmision/overlay → OBS Overlay que muestra el countdown con el acento/color correcto
        window.open('/transmision/overlay', 'OBS_WINDOW', 'width=1920,height=1080');

        // Broadcast Supabase en background (para pantallas ya abiertas)
        // NO esperamos - cerramos el modal inmediatamente para mejor UX
        const broadcastAsync = async () => {
            try {
                const channel = supabase.channel('obs_countdown_launch_' + Date.now());
                await new Promise((resolve) => {
                    ...
                    channel.subscribe((status) => {
                        if (status === 'SUBSCRIBED') {
                            channel.send({ type: 'broadcast', event: 'update_overlay', payload: countdownPayload })
                                .then(() => {
                                    clearTimeout(timer);
                ...
                // Ignorar errores de broadcast; las ventanas ya leen desde localStorage
            }
        };
        broadcastAsync();
